...
test.describe("drawn site boundary Finish workflow", () => {
  ...
  test("draft precision controls support coordinates, snaps, transforms, layers, and history", async ({ page }) => {
    await openBlankWorkspace(page);
    ...
    await cadTools.getByLabel("Draft command input").fill("80,90");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(page.getByTestId("cad-active-command")).toContainText("1/2+");
    await cadTools.getByLabel("Draft command input").fill("180,90");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("LINE accepted 2 points");
    await cadTools.getByLabel("Draft command input").fill("");
    await cadTools.getByRole("button", { name: "Run" }).click();
    ...
    await cadTools.getByLabel("Draft X coordinate").fill("120");
    ...
    await cadTools.getByLabel("Draft command input").fill("10");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("OFFSET applied 10 ft");
    await cadTools.getByLabel("Draft command input").fill("EXTEND");
    await cadTools.getByRole("button", { name: "Run" }).click();
    ...
    await cadTools.getByLabel("Draft layer").selectOption("C-UTIL");
    await clickVisibleControl(cadTools.getByRole("button", { name: "Layer" }));
    await expect(cadTools).toContainText("Layer");
    await cadTools.getByLabel("Draft command input").fill("SELECT NONE");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("SELECT NONE cleared");
    await cadTools.getByLabel("Draft command input").fill("SELECT LAYER C-UTIL");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText(/SELECT LAYER C-UTIL selected \d+/);
    await cadTools.getByLabel("Draft command input").fill("M 1");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("MOVE applied");
    await cadTools.getByLabel("Draft command input").fill("RO 5");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("ROTATE applied");
    await cadTools.getByLabel("Draft command input").fill("SC 1.01");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("SCALE applied");
    await cadTools.getByLabel("Draft command input").fill("CO 2,2");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText(/COPY created .*draft review copy/);
    await cadTools.getByLabel("Draft command input").fill("ARRAY 2 3 12,8");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("ARRAY created 5 draft review copies");
    await cadTools.getByLabel("Draft command input").fill("DIST 0,0 100,0");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("DIST 100.00 ft");
    await cadTools.getByLabel("Draft command input").fill("MEASURE");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText(/DIST selected .* ft total/);
    await cadTools.getByLabel("Draft command input").fill("MI H");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("MIRROR H applied");
    await cadTools.getByLabel("Draft command input").fill("FLIP V");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("MIRROR V applied");
    await cadTools.getByLabel("Draft dimension mode").selectOption("aligned");
    await cadTools.getByLabel("Draft dimension label").fill("130.0 ft review");
    ...
    await cadTools.getByLabel("Draft command input").fill("move 5");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("MOVE applied");
    await cadTools.getByLabel("Draft command input").fill("MOVE");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(page.getByTestId("cad-active-command")).toContainText("Active command: MOVE");
    await cadTools.getByLabel("Draft command input").fill("6,0");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("MOVE applied 6,0");
    await cadTools.getByLabel("Draft command input").fill("ROTATE");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(page.getByTestId("cad-active-command")).toContainText("Active command: ROTATE");
    await cadTools.getByLabel("Draft command input").fill("15");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("ROTATE applied");
    await cadTools.getByLabel("Draft command input").fill("SCALE");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(page.getByTestId("cad-active-command")).toContainText("Active command: SCALE");
    await cadTools.getByLabel("Draft command input").fill("1.05");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText("SCALE applied");
    await cadTools.getByLabel("Draft command input").fill("COPY");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(page.getByTestId("cad-active-command")).toContainText("Active command: COPY");
    await cadTools.getByLabel("Draft command input").fill("8,8");
    await cadTools.getByRole("button", { name: "Run" }).click();
    await expect(cadTools).toContainText(/COPY created .*draft review cop/);

    await cadTools.getByLabel("Draft X coordinate").fill("300");
    await cadTools.getByLabel("Draft Y coordinate").fill("140");
    await cadTools.getByLabel("Draft symbol", { exact: true }).selectOption("hydrant");
