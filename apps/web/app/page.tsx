          ...
          >
            ...
            <div className="mt-3 rounded-lg border border-slate-200 bg-white px-3 py-3 text-left text-slate-900">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                <button type="button" aria-label="Open canvas from sidebar command" onClick={() => handleOpenPanelFromDrawer("model")} className="rounded-md border border-slate-200 bg-slate-50 px-2 py-2 text-xs font-semibold text-slate-700 hover:bg-white">
                  Canvas
                </button>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4">
                ...
                {sidePanelForRender === "water" ? (() => {
                  const fmt = (value: unknown, suffix = "", digits = 0) => {
                    const next = typeof value === "number" ? value : Number(value);
                  ...
                  return (
                    <div className="space-y-4" data-testid="water-fire-flow-workbench">
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white p-4">
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-semibold text-slate-700">
                          {waterFireFlowReview.checkRows.map(([label, value]) => (
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-2">
                          ...
                          <button type="button" onClick={() => handleGenerateSystem("utilities")} className="rounded-xl border border-slate-950 bg-slate-950 px-3 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-white hover:bg-slate-800">Run water review</button>
                        </div>
                      </div>
                      <div className="rounded-2xl border border-slate-200 bg-white p-4">
                        ...
                        <div className="mt-3 max-h-48 overflow-auto rounded-xl border border-slate-200">
                          <table className="w-full text-left text-[11px]">
                            ...
                            <tbody>
                              {waterFireFlowReview.networkSegments.length ? waterFireFlowReview.networkSegments.slice(0, 8).map((segment) => (
                                <tr key={segment.id} className="border-t border-slate-100">
                                  <td className="px-2 py-1.5 font-semibold text-slate-900">{segment.label || segment.id}</td>
                                  <td className="px-2 py-1.5 capitalize">{String(segment.network_type || "review").replace("_", " ")}</td>
                                  <td className="px-2 py-1.5">{fmt(segment.velocity_fps, " fps", 1)}</td>
                                  <td className="px-2 py-1.5">{fmt(segment.end_pressure_psi, " psi", 1)}</td>
                                </tr>
                              )) : (
                                <tr>
                        ...
                        <div className="mt-3 max-h-40 overflow-auto rounded-xl border border-slate-200">
                          <table className="w-full text-left text-[11px]">
                            ...
                            <tbody>
                              {waterFireFlowReview.spacingChecks.length ? waterFireFlowReview.spacingChecks.map((row, index) => (
                                <tr key={`${row.from || "from"}-${row.to || "to"}-${index}`} className="border-t border-slate-100">
                                  ...
                                  <td className="px-2 py-1.5">{fmt(row.spacing_ft, " ft", 1)}</td>
                                  <td className="px-2 py-1.5">{fmt(row.limit_ft, " ft", 1)}</td>
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white p-4">
                        ...
                        <div className="mt-2 space-y-2">
                          {waterFireFlowReview.scenarios.length ? waterFireFlowReview.scenarios.map((scenario) => (
                            <div key={scenario.id} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-700">
                              <div className="flex items-center justify-between gap-2">
                              ...
                              <p className="mt-1">
                                Flow {fmt(scenario.available_flow_gpm, " gpm", 0)} / {fmt(scenario.required_flow_gpm, " gpm", 0)}; residual {fmt(scenario.residual_pressure_psi, " psi", 1)}.
                              </p>
                      ...
                      <div className="rounded-2xl border border-slate-200 bg-white p-4">
                        ...
                        <div className="mt-2 space-y-2">
                          {waterFireFlowReview.blockerCards.length ? waterFireFlowReview.blockerCards.map((blocker) => (
