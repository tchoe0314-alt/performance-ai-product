...
import {
  ADD_MENU_SECTIONS,
  SITE_OBJECT_CATALOG,
  requestedProgramToPendingPlacements,
...
<body>
  ...
  const preview3DItems = useMemo<Preview3DItem[]>(() => {
    ...
    for (const action of actions) {
      ...
      if (
        actionId &&
        (cadEntityPreview.sourceIds.has(actionId) || cadEntityPreview.linkedIds.has(actionId))
      ) {
        continue;
      }
  ...
  const handleUndoDraftAction = useCallback(() => {
    ...
    if (draftAction.action === "explode") {
      ...
      pushRecoveryMessage(`Undo: restored ${draftAction.object.label} after ${draftAction.label}.`);
      appendChatMessage("assistant", `Undo: restored ${draftAction.object.label} after ${draftAction.label}.`, "status");
      recordRecentChange({
        type: "object_added",
