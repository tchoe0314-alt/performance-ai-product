...
def _infer_buildings_from_legacy(parsed: Dict[str, Any], site_box: Rect) -> List[Dict[str, Any]]:
    ...
    if program_specs:
        ...
        if frontage_specs and len(primary_specs) == 3:
            margin_y = max(16.0, buildable["h"] * 0.035)
        ...
        if frontage_specs and len(primary_specs) == 3:
            upper_h = max(max(_safe_float(spec.get("d"), fallback_depth) for spec in primary_specs[:2]) + 2.0, min(vertical_span * 0.045, 36.0))
            middle_h = max(max(_safe_float(spec.get("d"), fallback_depth) for spec in primary_specs[2:]) + 2.0, min(vertical_span * 0.04, 32.0))
            lower_h = max(max(_safe_float(spec.get("d"), fallback_depth) for spec in frontage_specs) + 2.0, min(vertical_span * 0.035, 28.0))
