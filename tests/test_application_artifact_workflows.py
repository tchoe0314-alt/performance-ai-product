...
class ApplicationArtifactWorkflowsTest(unittest.TestCase):
    ...
    def test_build_preview_response_blocks_stored_run_errors(self):
        service = FakeArtifactService()
        response = build_preview_response(
            artifact_service=service,
            result_data={
                ...
                "final_plan": {
                    "project_name": "Errored Stored Run",
                    "actions": [{"layer": "LOT"}],
                    "meta": {},
                },
            },
        ...
        self.assertEqual(review["release_status"], "blocked")
        self.assertFalse(review["release_ready"])
        self.assertIn("planner_errors_present", review["blocked_reasons"])
    ...
    def test_build_preview_response_keeps_blocked_status_trace_with_other_blockers(self):
        service = FakeArtifactService()
        response = build_preview_response(
            artifact_service=service,
            result_data={
                ...
                "final_plan": {
                    "project_name": "Blocked Status With Reason",
                    "actions": [{"layer": "BUILDING"}],
                    "meta": {
                        "release_review": {
                            "release_status": "blocked",
    ...
    def test_export_dxf_artifact_blocks_release_review_missing_deliverables(self):
        ...
        result_data = {
            "final_plan": {
                "project_name": "Blocked Missing Report DXF",
                "actions": [{"task": "polyline", "layer": "LOT", "points": [[0, 0], [1, 0]]}],
                "meta": {
                    "release_status": "ready",
                    "release_ready": True,
        ...
        with self.assertRaises(HTTPException) as ctx:
            export_dxf_artifact(
                ...
                project_id="p1",
                result_data=result_data,
                filename_stem="blocked-missing-report",
            )

        self.assertEqual(ctx.exception.status_code, 409)
        self.assertIn("missing_deliverable_report", str(ctx.exception.detail))
        self.assertIsNone(service.dxf_export)
    ...
    def test_export_report_artifact_updates_project_workflow(self):
        ...
        self.assertEqual(service.report_export["result_data"]["final_plan"]["project_name"], "Test")
