...
class ApplicationJobWorkflowsTest(unittest.TestCase):
    ...
    def test_build_orchestrate_job_runner_honors_blocked_release_status_without_reasons(self):
        ...
        self.assertEqual(final_plan["release_status"], "blocked")
        self.assertFalse(final_plan["release_ready"])
        self.assertIn("release_status_blocked", final_plan["blockers"])
        self.assertFalse(release_review["release_ready"])
        self.assertIn("release_status_blocked", release_review["blocked_reasons"])
        self.assertFalse(final_plan["meta"]["phase_checkpoints"]["combined_view"]["ready"])
        saved_plan = store.saved_payload["latest_result"]["final_plan"]
        self.assertFalse(saved_plan["release_ready"])
        self.assertIn("release_status_blocked", saved_plan["blockers"])
    ...
    def test_build_orchestrate_job_runner_blocks_export_ready_when_requested_deliverable_is_missing(self):
        ...
        result = runner(
            {
                "job_id": "job_missing_deliverable",
                "job_type": "orchestrate",
                "user_id": "u1",
                "project_id": "p1",
                "payload": {"prompt_text": "run"},
    ...
    def test_build_orchestrate_job_runner_preserves_failed_deliverables_from_release_review(self):
        store = FakeProjectStore(
            {
                "user_id": "u1",
                "project_id": "p1",
                ...
                "final_plan": {
                    ...
                    "meta": {
                        ...
                        "release_review": {
                            ...
                            "release_status": "ready",
                            "requested_deliverables": ["site_plan", "report"],
                            "produced_deliverables": ["site_plan", "report"],
                            "failed_deliverables": ["report"],
                        },
                    },
                },
        ...
        runner = build_orchestrate_job_runner(
            ...
            run_orchestration=run_orchestration,
            build_run_summary=lambda result, **kwargs: {
                "run_id": "run_release_review_failed_deliverable",
                "job_id": kwargs.get("job_id"),
                "source": "queued_job",
                "convergence_summary": {
                    "assumption_summary": {"count": 0, "categories": [], "examples": []},
        ...
        result = runner(
            {
                "job_id": "job_release_review_failed_deliverable",
                "job_type": "orchestrate",
                "user_id": "u1",
                "project_id": "p1",
                "payload": {"prompt_text": "run"},
        ...
        self.assertFalse(final_plan["release_ready"])
        self.assertFalse(final_plan["export_ready"])
        self.assertEqual(final_plan["deliverables"]["requested"], ["site_plan", "report"])
        self.assertEqual(final_plan["deliverables"]["produced"], ["site_plan"])
        self.assertEqual(final_plan["deliverables"]["failed"], ["report"])
        self.assertEqual(final_plan["deliverables"]["ready"], ["site_plan"])
        self.assertIn("failed_deliverable_report", final_plan["blockers"])
        self.assertIn("failed_deliverable_report", release_review["blocked_reasons"])
        self.assertEqual(release_review["failed_deliverables"], ["report"])
        self.assertEqual(release_review["produced_deliverables"], ["site_plan"])
        self.assertEqual(final_meta["run_summary"]["failed_deliverables"], ["report"])
        saved_plan = store.saved_payload["latest_result"]["final_plan"]
        self.assertFalse(saved_plan["release_ready"])
        self.assertEqual(saved_plan["deliverables"]["failed"], ["report"])
    ...
    def test_build_orchestrate_job_runner_blocks_reactive_post_rerun_release_failures(self):
        store = FakeProjectStore(
            {
                "user_id": "u1",
                "project_id": "p1",
        ...
        runner = build_orchestrate_job_runner(
            ...
            run_orchestration=run_orchestration,
            build_run_summary=lambda result, **kwargs: {
                "run_id": "run_reactive_release_failure",
                "job_id": kwargs.get("job_id"),
                "source": "queued_job",
                "convergence_summary": {
                    "assumption_summary": {"count": 0, "categories": [], "examples": []},
        ...
        result = runner(
            {
                "job_id": "job_reactive_release_failure",
                "job_type": "orchestrate",
                "user_id": "u1",
                "project_id": "p1",
                "payload": {"prompt_text": "run"},
        ...
        self.assertFalse(final_plan["release_ready"])
        self.assertFalse(final_plan["export_ready"])
        self.assertIn("reactive_post_rerun_not_ready", final_plan["blockers"])
        self.assertIn("manual_validation_manual_storm_hydraulic_invalid", final_plan["blockers"])
        self.assertFalse(release_review["release_ready"])
        self.assertIn("reactive_post_rerun_not_ready", release_review["blocked_reasons"])
        self.assertEqual(final_meta["phase_checkpoints"]["combined_view"]["status"], "blocked")
        self.assertFalse(final_meta["phase_checkpoints"]["combined_view"]["ready"])
        saved_plan = store.saved_payload["latest_result"]["final_plan"]
        self.assertFalse(saved_plan["release_ready"])
