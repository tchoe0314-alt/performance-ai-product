...
class DxfSheetLayoutsTest(unittest.TestCase):
    ...
    def test_save_dxf_creates_sheet_layouts(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertFalse(audit.get("success"))
            self.assertFalse(audit.get("production_export_ready"))
            self.assertTrue(audit.get("export_blocked"))
            self.assertIn("construction_readiness_blocked", audit.get("blocked_reasons") or [])
