...
class LayoutFallbackActionsTests(unittest.TestCase):
    def test_synthesized_program_layout_keeps_frontage_row_clustered(self) -> None:
        ...
        nearest_multifamily_y = min(
            float(item["y"]) + float(item["d"]) / 2.0 for item in multifamily
        )
    ...
    def test_layout_fallback_keeps_parking_modules_proportional(self) -> None:
        ...
        self.assertLess(
            max(float(action.get("origin", [0.0, 0.0])[1]) for action in retail_parking),
            min(float(action.get("origin", [0.0, 0.0])[1]) for action in multifamily_parking),
        )
