...
from backend.planning.gis_provider_registry import build_provider_registry, check_registry_health
...
def upload_existing_conditions_file(
    *,
    ...
    package_meta = {
        ...
        "point_clouds": merged.get("point_clouds"),
        "sources": merged.get("sources"),
        "metadata_only_sources": merged.get("metadata_only_sources"),
        "canonical_targets": merged.get("canonical_targets"),
        "canonical_existing_conditions_model": merged.get("canonical_existing_conditions_model"),
    ...
    return {
        ...
        "import_validation": merged.get("import_validation"),
        "import_matrix": (merged.get("import_validation") or {}).get("import_matrix") or (merged.get("import_validation") or {}).get("importer_production_matrix"),
        "canonical_vs_metadata_only": (merged.get("import_validation") or {}).get("canonical_vs_metadata_only"),
        "blockers": (merged.get("import_validation") or {}).get("blockers"),
        "blocker_details": (merged.get("import_validation") or {}).get("blocker_details"),
...
def existing_conditions_online_sources(
    *,
    ...
    registry = build_provider_registry(providers=(provider_registry or {}).get("providers", []) if isinstance(provider_registry, dict) else [])
    return {
        "success": True,
...
def fetch_existing_conditions_online(
    *,
    ...
    if include_contours and (contours_service_url or contours_layer_id):
        result.setdefault("warnings", []).append("Configured contour ArcGIS fetch is not wired into the online fetcher yet.")
...
def build_local_gis_provider_registry(*, providers: Optional[list[Dict[str, Any]]] = None) -> Dict[str, Any]:
    return build_provider_registry(providers=providers or [])


def check_local_gis_provider_registry(*, providers: Optional[list[Dict[str, Any]]] = None) -> Dict[str, Any]:
    return check_registry_health(build_provider_registry(providers=providers or []))
